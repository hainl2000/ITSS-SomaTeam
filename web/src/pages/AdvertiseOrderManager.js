...
export default function AdvertiseOrderManager() {
  ...
  const allPackage = useQuery(['adminGetAllTransactionAds'], () =>
    ProductAPI.adminGetAllAdvertise()
  );
              ...
              {allPackage.isLoading ? (
                ...
                <Tbody>
                  {allPackage?.data?.map((item) => (
                    <Tr key={item.id}>
                      <Td>{item?.advertise_package?.name}</Td>
                      <Td>{item?.advertise_package?.price}</Td>
                      <Td>
                        {new Date(
                          item?.created_at
                        ).toLocaleDateString()}
